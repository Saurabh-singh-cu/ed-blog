  ...
  const fetchUserResponse = async () => {
    try {
      const fetchApi = await fetch(
        "https://apiedportfolio.unicornfortunes.com/text-post/all/"
      );
        ...
        >
          <div className="title-table-container">
            ...
            <button
              ...
              type="button"
              class="text-white bg-[#9ddad4] hover:bg-[#FF9119]/80 focus:ring-4 focus:outline-none focus:ring-[#FF9119]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:hover:bg-[#FF9119]/80 dark:focus:ring-[#FF9119]/40 me-2 mb-2"
            >
              <img style={{ width: "30px", marginRight: "10px" }} src={gmail} />
              Send Mail
            </button>
          </div>
          <div>
                ...
                {users &&
                  users.map((item, id) => (
                    <tr key={id}>
                      ...
                      <td>{item?.title}</td>
                      <td> {item?.email}</td>
                      <td className="message-cell" title={item?.content}>
